...
def eval_ASR(model, test_loader, args, val_attack):
    ...
    save_attack_pairs = getattr(args, 'save_attack_pairs', False)
    attack_pairs_dir = getattr(args, 'attack_pairs_dir', 'saved_hitadv_pairs')
    if save_attack_pairs:
    ...
    for i, (ori_data, label) in enumerate(pbar):
        ...
        clean_pair_data = ori_data[:, :, :3].detach().cpu().numpy().astype(np.float32)
        adv_pair_data = adv_data.detach().cpu().numpy().astype(np.float32)
        ...
        with torch.no_grad():
            ...
            if save_attack_pairs:
                np.savez_compressed(
                    os.path.join(attack_pairs_dir, f'batch_{batch:06d}.npz'),
                    clean=clean_pair_data,
                    adv=adv_pair_data,
                    label=label.detach().cpu().numpy().astype(np.int64),
                    ori_pred=ori_preds.detach().cpu().numpy().astype(np.int64),
                    adv_pred=adv_preds.detach().cpu().numpy().astype(np.int64),
                    attack_success=(mask_ori & (~mask_adv)).detach().cpu().numpy().astype(bool),
                )
